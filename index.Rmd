...
Tally the number of cases per state that have been recorded in captive heards
```{r}
...
Get a spatial map for the counties
```{r}
t_counties <- tigris::counties(cb = TRUE)
head(t_counties)
```
...
# labs <- lapply(wildmap_tigris$mytext2,htmltools::HTML)
# labels <- sprintf(
#   "%s County",wildmap_tigris$NAMELSAD
